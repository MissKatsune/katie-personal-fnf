...
--Disable perfects if you are on OS engine--
...
local font = 'sonic-1-hud-font.ttf' -- H24's font is 'goodbyeDespair.ttf' but 'sonic-1-hud-font.ttf' is pretty cool
...
local OSversion = 'Psych' --Psych/Anything else (Anything else makes it grab the ms from OS's playstate) (Psych one is broken, i no longer have time to do things)
local Ourple = false; --ourples the score

--Timers--

local timer = 2.05; --default is 2.05 --This is how long it takes to deplete
local timeItTakesToSpeedUp = 2; --default is 2 --pretty self explanatory
local speedUpMult = 2; --default is 2 --multiplier of the speed up

--Size--

local scoresize = 35; --default is 35 (set to 25 if using goodbyeDespair font) --score text size
local size = 50; --default is 50 --size of the special text

--Animations--

local AnimationReal = true; --makes bf do the hey thing when the combo is done (and perfect) (default: true)
...
local enableGfAnimCheer = true; -- Yeah
local enableGfAnimSad = true; -- Yeah
...
--Custom Texts--
local textSetting = 'base' --base, custom, or both. changes what text to use. both is randomized each note hit
local customRateText = {'EPIC!', 'COOL!', 'BAD', 'SHIT'} --Custom Text Here
local allText = {'SICK!', 'NICE!', 'BAD', 'SHIT', 'EPIC!', 'COOL!', 'BAD', 'SHIT'} --Custom is the last 4
local customTextChance = 50; --The chance of the custom text appearing when textSetting is both
...
local bad = 0;
local shit = 0;
local perfect = 0; --I must remind you again to remove the Perfect Rating on OS Engine
...
function onCreate()
    ...
    if songName == 'Sugarcrush' or songName == 'Thunderstorm' and getProperty('storyDifficultyText') == 'hard' or songName == 'Revenge-4k' then
...
function onCreatePost()
    if downscroll and enabled then
        ...
        makeLuaText('comboCool', 'x'..recentCombo, 0, 660, 550)
        setObjectCamera('comboCool', 'hud')
        setTextAlignment('comboCool', 'center')
        addLuaText('comboCool')
    ...
    if downscroll and middlescroll and enabled then
        ...
        makeLuaText('comboCool', 'x'..recentCombo, 0, 650, 150)
        setObjectCamera('comboCool', 'hud')
        setTextAlignment('comboCool', 'center')
        addLuaText('comboCool')
    ...
    if not downscroll and enabled then
        ...
        makeLuaText('comboCool', 'x'..recentCombo, 0, 670, 50)
        setObjectCamera('comboCool', 'hud')
        setTextAlignment('comboCool', 'center')
        addLuaText('comboCool')
    ...
    if not downscroll and middlescroll and enabled then
        ...
        makeLuaText('comboCool', 'x'..recentCombo, 0, 650, 450)
        setObjectCamera('comboCool', 'hud')
        setTextAlignment('comboCool', 'center')
        addLuaText('comboCool')
...
function onUpdate(elapsed)
    ...
    elseif textStyle == 'OS' then
        ...
        if MSversion == 'Psych' then
            FinalScore = 'SCORE: '..visibleScore..' | COMBO BREAKS: '..misses..' | AVERAGE: '..round(averageMS, 0)..'ms | Accuracy: '..rateFullPer..'% | '..ratingName..' ['..ratingFC..']'
            if Ourple then
                FinalScore = 'HOW OURPLE YOU ARE: '..visibleScore..' | COMBO BREAKS: '..misses..' | AVERAGE: '..round(averageMS, 0)..'ms | Accuracy: '..rateFullPer..'% | '..ratingName..' ['..ratingFC..']'
            end
        else
    ...
    elseif textStyle == 'Yoshi-ish' then
        ...
        if MSversion == 'Psych' then
            FinalScore = 'SCORE: '..visibleScore..' | COMBO BREAKS: '..misses..' | Accuracy: '..rateFullPer..'% ('..ratingName..') | ['..ratingFC..'] AVERAGE: '..round(averageMS, 0)..'ms'
            if Ourple then
                FinalScore = 'HOW OURPLE YOU ARE: '..visibleScore..' | COMBO BREAKS: '..misses..' | Accuracy: '..rateFullPer..'% ('..ratingName..') | ['..ratingFC..'] AVERAGE: '..round(averageMS, 0)..'ms'
            end
        else
    ...
    if fcStatus ~= 1 then
        if visibleScore >= 25000 and visibleScore < 35000 then
    ...
    if scoreToAdd < 0 then
        ...
        doTweenAlpha('glowShrink', 'glower', baseGlowOpaceity, 0.2, 'linear')
...
function goodNoteHit(id, direction, noteType, isSustainNote)
    if not isSustainNote then
        ...
        if scoreZoom and not deactivateSystemZoom or SystemZoom then
            setProperty('daRate.scale.x', 1.075)
	        setProperty('daRate.scale.y', 1.075)
            setProperty('comboCool.scale.x', 1.075)
	        setProperty('comboCool.scale.y', 1.075)
	        ...
	        doTweenX('ratingX', 'daRate.scale', 1, 0.2, 'linear')
	        doTweenY('ratingY', 'daRate.scale', 1, 0.2, 'linear')
            doTweenX('XX', 'comboCool.scale', 1, 0.2, 'linear')
	        doTweenY('XY', 'comboCool.scale', 1, 0.2, 'linear')
